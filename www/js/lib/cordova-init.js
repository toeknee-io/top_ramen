(function cordovaInitIife({ console, TopRamenApi,
  ...
  const cordovaApp = {
    ...
    onDeviceReady() {
      ...
      */
      console.log('on device rdy, platform: %s', window.device.platform);

      document.addEventListener('backbutton', onBackKeyDown, false);

      const trApi = window.trApi = window.tra = new TopRamenApi({ cordovaApp });

      if (window.PushNotification && window.device.platform !== 'ios') {
